...
import {
  DEFAULT_LOBBY, NETWORK_VERSION, RESERVATION_MS, ROOM_IDLE_MS, ROOM_TTL_MS, SIGNAL_MAX_BYTES,
  byteLength, json, parseClientMessage, type ClientMessage, type Participant, type Room, type ServerMessage,
...
/** Shared state machine. No race state or race tick ever enters this service. */
export class RoomEngine {
  ...
  constructor(code: string, private readonly now: () => number = Date.now, saved?: StoredRoom) {
    this.state = saved ?? {
      room: { version: NETWORK_VERSION, code, revision: 0, expiresAt: now() + ROOM_TTL_MS, phase: "lobby",
        ...
        config: { ...DEFAULT_LOBBY }, startAt: null, seed: 0, checkpoint: null, pendingCheckpoint: null, reason: null },
  ...
  private command(p: Participant, connectionId: string, m: Exclude<ClientMessage, { type: "join" }>): boolean {
    ...
    switch (m.type) {
      ...
      case "config": host(); lobby(); this.room.config = m.config; this.clearReady(); return true;
      case "start": {
        host(); lobby();
      ...
      case "migration-ready": {
        ...
        this.room.phase = "racing"; this.room.reason = "Authority transferred from the last committed checkpoint.";
        this.room.startAt = this.now() + 1000; this.state.migrationDeadline = null; return true;
      }
      case "migration-failed": epoch(m.epoch); authority(); this.returnToLobby("Authority transfer could not restore the committed checkpoint. No race results were generated."); return true;
      case "transport-failed": {
        ...
        if (this.room.phase !== "lobby") this.returnToLobby(reason);
  ...
  private commitIfReplicated(): void {
    ...
    if (this.room.pendingCheckpoint && (this.state.pendingHolders.length >= 2 || connected.length === 1)) {
      ...
      this.state.holders = [...this.state.pendingHolders];
  ...
  private checkAuthority(): void {
    ...
    this.returnToLobby("Authority was lost without a valid replicated checkpoint. Everyone stays in this lobby; no race results were generated.");
  }
  ...
  tick(): void {
    ...
    if (this.state.migrationDeadline !== null && this.now() >= this.state.migrationDeadline) {
      this.returnToLobby("Authority transfer timed out. The race was not finished; ready again in the same lobby."); changed = true;
    }
  ...
  private send(connectionId: string, message: ServerMessage): void {
    try { this.connections.get(connectionId)?.socket.send(message); }
